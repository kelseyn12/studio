...
- `components/` shell, nav, pills, action card, studio map, today board, plan board, hook rewrite, refresh stats, batch settings, batch outputs, text style pick, clip tile, editor need, packet files, calendar board, day slot, post chip, card brief/footage/editor/live, paid button, failed posts, deal edit, storage meter, library deals/file, delete video
- `lib/` prisma, pipeline, deals, next-action, session, Clerk, R2, storage, library-groups, cut-ready, Outstand, analytics, formats, rewrite, whisper, script, pull-media, manychat, ffmpeg, variations, combinations, editor-packet, queries, publish, media-url, render-batch, captions, batch-polish, ingest, text-style
