# Refactoring log
- 2026-09-25 — Platform text looks for Multiply hooks (lib/text-style.ts): TikTok (shadow), Instagram (per-line box), Plain. `textStyle` on the batch, "auto" follows the account network. Hooks now wrap at ~26 chars and draw one centered drawtext per line (works on Fly's ffmpeg 5.1 too). Picker split into components/text-style-pick.tsx to keep batch-settings from growing.
- 2026-09-25 — Uploads over 10MB were 500ing: Next 15.5 buffers bodies behind middleware and drops anything bigger (`middlewareClientMaxBodySize`, now 1gb). Per-file cap moved to `STUDIO_FILE_MAX_MB` (1GB default, 250 on Fly). Multiply clips now go through lib/ingest.ts: ffprobe the size, shrink 4K to a 1080 short side before R2, refuse files ffprobe cannot open. `saveUpload` split into `saveLocalUpload` + `uploadLocalToR2`.
...
- 2026-09-20 — Deals have a kind (Canvas/tech vs traditional UGC). Today shows money + both lanes. Send to editor redirects to CapCut in. Income and Rules stay folded in, not extra apps.
- 2026-09-19 — Cut the Base44 maze: two calendars stay, but Plan month is film-only and Live is publish-only. Card desk is four rooms. Today dropped vanity stats. Nav is Do + Desk.
- 2026-09-19 — Calendar is the posting control: week navigation, Week/Month/Scheduled/Posted, Add slot per day, bulk auto-space. Local date parse so a picked day does not shift in UTC.
- 2026-09-19 — Editor login only opens CapCut in and assigned cards. Assign from the card after adding them on Team.
