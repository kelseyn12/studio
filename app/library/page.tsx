...
export default async function LibraryPage() {
  ...
  return (
    <Shell>
      ...
      <p className="mt-2 mb-8 text-mute">Finished videos only. CapCut exports and repurposed cuts live here.</p>
        ...
        {assets.length === 0 ? (
          ...
          assets.map((asset) => (
            <article key={asset.id} className="rounded-card border border-line bg-panel p-4">
              <video controls src={publicFileUrl(asset.path)} className="mb-3 aspect-[9/16] max-h-80 w-full rounded-xl bg-ink object-cover" />
